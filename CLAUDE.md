...
node --test __tests__ server/__tests__ server/proxy/__tests__    # 跑全部 JS 测试
node --test server/__tests__/stripe-verify.test.js               # 跑单个文件
node --test --test-name-pattern='extractCsId' server/__tests__/stripe-verify.test.js  # 按名字过滤

# 协议模式 Python 测试（unittest）
...
仓库**没有**配置 lint/format 工具（无 eslint/prettier/black 配置），也没有 `npm test` 脚本；运行测试请直接调 `node --test`。
...
- **`server/db.js`** —— `sql.js`（WASM SQLite）+ `data.db` 文件。三张表：`accounts` / `account_status` / `execution_logs`。每次写都 `db.export()` 回写整个文件。包含从老 status 值到当前命名（`plus` / `plus_no_rt` / `no_link` / `error` / `idle` / `running` + v2.19 新增 `no_jp_proxy` / `no_promo` / `verify_error`）的一次性迁移逻辑。
...
- **Socket.IO** —— 引擎事件直接 `io.emit` 到前端；CORS 锁死 `localhost:3000` / `127.0.0.1:3000`，不做认证（设计上仅本机用）。
...
- **Chrome 由 Node 启动，Playwright 用 CDP 连** —— `server/chrome.js` 用 `spawn` 启 `chrome.exe --remote-debugging-port=...`（找路径优先级见 `CHROME_PATHS`），然后 `chromium.connectOverCDP`。`engine.stop()` 必须 `_chromeProc.kill()` + `_browser.close()` + `fs.rmSync(_tempDir)`，否则会留临时 user-data-dir。
