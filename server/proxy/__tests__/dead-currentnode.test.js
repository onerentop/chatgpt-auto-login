...
test('A1 currentNode alive=false 触发 fire-and-forget rotate', { skip: 'v2.42 Task 8: __autoRotateIfCurrentDead / probeResults 已删' }, async () => {
  const calls = []
...
test('A2 currentNode alive=true 不触发 rotate', { skip: 'v2.42 Task 8: __autoRotateIfCurrentDead 已删' }, async () => {
  const calls = []
...
test('A3 currentNode 没在 probeResults（未探过）不触发 rotate', { skip: 'v2.42 Task 8: __autoRotateIfCurrentDead 已删' }, async () => {
  const calls = []
...
test('A4 currentNode 为空串不触发 rotate', { skip: 'v2.42 Task 8: __autoRotateIfCurrentDead 已删' }, async () => {
  const calls = []
