...
test('O1 takeOrder: 成功返 { phone, remaining }', async () => {
  const restore = mockFetch(async (url, opts) => {
    assert.ok(url.includes('action=open_get_phone'));
    assert.strictEqual(opts.headers['X-API-Key'], 'SMS-AAAA-BBBB-CCCC');
  ...
  try {
    ...
    const r = await oapi.takeOrder('SMS-AAAA-BBBB-CCCC', null);
...
test('O2 takeOrder: ok=false 抛错带 _oapiCode', async () => {
  ...
  try {
    ...
    await assert.rejects(
      oapi.takeOrder('SMS-X', null),
      (e) => e.message.includes('无效的CDK兑换码') && e._oapiCode === 'api_fail'
    );
  } finally { restore(); }
});

test('O3 pollOnce: ok=true 返 { code, remaining }', async () => {
  ...
  try {
    delete require.cache[require.resolve('../oapi-provider')];
    const oapi = require('../oapi-provider');
    const r = await oapi.pollOnce('SMS-X', null);
...
test('O5 changePhone: 成功返 { phone, remaining }', async () => {
  ...
  try {
    delete require.cache[require.resolve('../oapi-provider')];
    const oapi = require('../oapi-provider');
    const r = await oapi.changePhone('SMS-X', null);
