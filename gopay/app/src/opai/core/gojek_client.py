...
class GojekClient:
    ...
    def initiate_otp(
        ...
        body = {
            ...
            "country_code": country_code or "+62",
            "email_address": None,
            "client_id": CLIENT_ID,
            "phone_number": phone or "",
            "client_secret": CLIENT_SECRET,
            "is_multiple_method": is_multiple_method if is_multiple_method else None,
    ...
    def login(self, country_code: str, phone: str, pin: str, otp_callback=None) -> dict:
        ...
        """
        # Login starts a fresh auth session — clear existing token so SSO
        # endpoints don't receive a stale/signup Authorization header.
    ...
    def accept_consents(self, flow: str = "signIn") -> dict:
        """POST /api/v2/consents/accept → accept TnC and privacy consents.

        HAR-captured (2026-05-29): GoPay app calls this on first login.
        The 1 Rp initial balance is credited by the backend after this call.
        """
        body = {
            "consents": [
                {"consent_name": "gopay_app_tnc", "user_type": "CUSTOMER", "flow": flow},
                {"consent_name": "gopay_app_privacy_note", "user_type": "CUSTOMER", "flow": flow},
                {"consent_name": "gojek_app_tnc", "user_type": "CUSTOMER", "flow": flow},
                {"consent_name": "gojek_app_privacy_note", "user_type": "CUSTOMER", "flow": flow},
            ]
        }
